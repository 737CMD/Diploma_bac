...
def main(params_file, results_file):
    ...
    airfoil = airfoil_generator.airfoil_structured(
        r_forw, up_coords, down_coords, teta, cfg['dy_te'], cfg['min_thick']
    )
    
    if airfoil.self_intersect():
        print("[Worker] Профиль самопересекается. FAIL.")
        write_results(results_file, failed=True)
        return

    airfoil.create_icem_file(500)

    # 2. Построение сетки в ICEM
    if not ir.run_icem(icem_exe, rpl_script_path):
        print("[Worker] Ошибка ICEM. FAIL.")
        write_results(results_file, failed=True)
        return
    ...
    try:
        session = pyfluent.connect_to_fluent(
            ip=conn['ip'], port=conn['port'], password=conn['password']
        )
        ...
        if cl is None or cd is None or cd >= 1:
            print("[Worker] Ошибка расчета во Fluent (или развал решения). FAIL.")
            write_results(results_file, failed=True)
            return
            
    except Exception as e:
        print(f"[Worker] Ошибка связи с Fluent: {e}. FAIL.")
        write_results(results_file, failed=True)
        return

    # 4. Возврат результата
    objective_value = (cd * 10000 + 10 * fine_function**2)
    print(f"[Worker] Успех! Cd*10k = {cd*10000:.2f}. Отдаем DAKOTA: {objective_value:.2f}")
